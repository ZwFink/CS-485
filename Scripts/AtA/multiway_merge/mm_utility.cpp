...
{
	uint64_t index, piv_index, curr_end_index, sublist_begin;
    ...
    for( index = 1; index < list_begin_ptrs->size(); ++index )
    {
        ...
        for( piv_index = 0; piv_index < first_sublist_ends->size() - 1; ++piv_index )
        {
            ...
            if( piv_index == 0 )
            {
                temp_start.push_back( sublist_begin );
            }
        ...
        temp_start.push_back( temp_end[ piv_index - 1 ] + 1 );
        temp_end.push_back( sublist_begin + sublist_size - 1 );
...
std::vector<uint64_t*> *generate_k_sorted_sublists( uint64_t *base_ptr, uint64_t total_elements, unsigned int seed, uint16_t k )
{
    ...
    #pragma omp parallel for shared( base_ptr, elements_per_list) private( batch_index ) 
    for( batch_index = 0; batch_index < k; ++batch_index )
        {
            std::sort( base_ptr + ( batch_index * elements_per_list ),
                       base_ptr + ( batch_index * elements_per_list ) + elements_per_list
                     );
            (*list_ptrs)[ batch_index ] = ( base_ptr + ( batch_index * elements_per_list ) );
        }
    return list_ptrs;
}
