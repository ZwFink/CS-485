...
int main( int argc, char **argv )
{
   ...
   error_code = cudaStreamCreate( &stream );	
	...
	{
		cout << "Error creating cuda stream" << endl;
	}
	...
	int outer_index = 0;
		...
		while( num_items < upper_bound )
		{
            ...
            double tstart = omp_get_wtime();            
				...
				error_code = cudaMemcpyAsync( dev_A, host_A, sizeof( char ) * num_items, cudaMemcpyHostToDevice, stream );

	         double tend = omp_get_wtime();
				
				double byte_size = sizeof( char ) * num_items;				
	
	         printf( "Time(HtoD)   %f   DataSize(Bytes):	%f\n", tend - tstart, byte_size );
